/**
 * Arcs to Bangkok — 38 origin cities, arcs converging on Bangkok.
 *
 * Extracted verbatim from the Remarcable Living arrival page. The only change
 * to the original is that the canvas is passed in rather than looked up by
 * selector, so more than one instance can exist on a page.
 *
 * No dependencies. Canvas 2D only.
 *
 *   import { mountArcs } from "./arcs-to-bangkok";
 *   const handle = mountArcs(canvasEl);   // starts immediately
 *   handle.destroy();                     // on unmount
 *
 * The landmass is a 60x25 bitmask at 6-degree cells, verified against 21 known
 * cities on land and 9 open-ocean points. It is rasterised once to an offscreen
 * canvas, never redrawn per frame.
 *
 * Honours prefers-reduced-motion: those visitors get the final composed frame
 * immediately rather than a blank canvas.
 */
export function mountArcs(canvasEl, options) {
  if (!canvasEl) return { destroy() {} };
  var opts = options || {};
  var DUR = opts.durationMs || 3800;
  ...
  /* ---------- world canvas ---------- */
  var wc=canvasEl, wx=wc?wc.getContext('2d'):null;
  var W=0,H=0,P=null;
  ...
  var raf = 0, t0 = null, dead = false;
  function frame(ts) {
    if (dead) return;
    if (t0 === null) t0 = ts;
    var el = ts - t0, t = Math.min(1, el / DUR);
    wdraw(t, t >= 1 ? el : undefined);
    raf = requestAnimationFrame(frame);
  }
  function onResize() { if (!dead) { wsetup(); buildLand(); wdraw(1, 0); } }
