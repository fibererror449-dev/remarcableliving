...
export type DraftPayload = {
  ...
  description?: string; sourceUrl?: string; provenance?: string; media?: Attachment[]; coverId?: string;
};
...
const strings: Record<string,number> = {reference:120,name:200,district:120,floor:80,stationName:120,lastVerified:10,description:10000,sourceUrl:2000,provenance:8000,coverId:100};
...
export function validatePayload(value: unknown): {payload?:DraftPayload;errors:string[];blockers:string[]} {
  ...
  const payload={...row,reference:typeof row.reference==='string'?row.reference.trim():row.reference,name:typeof row.name==='string'?row.name.trim():row.name} as DraftPayload;
